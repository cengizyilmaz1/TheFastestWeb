...
The private restore rehearsal is documented separately and never runs through HTTP. The current migration chain contains nine migrations through `0008_indietools_categories`; see [DATABASE.md](DATABASE.md). At the historical `0007` checkpoint, migrations `0000`–`0006` and their fingerprints were unchanged, and the rehearsal preserved all 42 pre-existing public tables and seven prior migration entries, producing 43 public tables without creating invitation data. No production migration, provider mail, payment mutation or destructive customer-data operation is part of the test suite.
