...
The original public UI has been restored. `/dashboard` and the separate founder pages are not present. Submission and purchase flows use `/submit` and `/pricing`; `/profile/[userId]` publishes only explicitly opted-in profile information. Pricing preserves the original offers, displays compatible active catalog pricing and disables unavailable purchases. A redirect or success query parameter does not grant access.
