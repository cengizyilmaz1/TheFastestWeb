...
4. Preserve the original dump offline. Prepare restore SQL privately, restore into an isolated database, run the guarded migration through 0006, and compare every old column, ownership link, sequence value and prior migration entry. Fail on any unexplained difference. Grant only reviewed runtime privileges.
