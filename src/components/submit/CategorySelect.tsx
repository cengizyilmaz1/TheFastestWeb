...
export function CategorySelect({ id, value, onChange }: { id: string; value: string; onChange: (value: string) => void }) {
  return (
    <div className="mb-3.5">
      <label htmlFor={id} className="block text-[0.75rem] font-semibold text-text-secondary mb-1">Category</label>
      <select id={id} name="category" value={value} onChange={(event) => onChange(event.target.value)}
