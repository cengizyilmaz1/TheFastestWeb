...
import { adInventory, adReservations, adSlots, checkoutOrders, entitlements, providerPayments, sites } from "@/db/schema";
...
export async function expireAdReservations(tx?: Transaction, inventoryId?: string) {
  const expire = async (transaction: Transaction) => {
    const expired = await transaction.update(adReservations).set({ status: "expired", updatedAt: sql`now()` })
      .where(and(eq(adReservations.status, "active"), sql`${adReservations.endsAt} <= now()`, inventoryId ? eq(adReservations.inventoryId, inventoryId) : sql`true`)).returning({ adSlotId: adReservations.adSlotId });
...
export async function listAvailableAdInventory() {
  ...
  const rows = await database().execute<{ id: string; position: "left" | "right"; orderIndex: number }>(sql`SELECT i.id,i.position,i.order_index AS "orderIndex"
    FROM ad_inventory i WHERE i.active AND NOT EXISTS (SELECT 1 FROM ad_reservations r WHERE r.inventory_id=i.id
      AND (r.status IN ('held','paid') OR (r.status='active' AND (r.ends_at IS NULL OR r.ends_at>now()))))
...
export async function reserveAdInventory(tx: Transaction, input: { inventoryId: string; orderId: string; userId: string; siteId: string }) {
  ...
  const [held] = await tx.select({ id: adReservations.id }).from(adReservations).where(and(eq(adReservations.inventoryId, inventory.id), inArray(adReservations.status, ["held", "paid", "active"])));
...
export async function approveAdReservation(tx: Transaction, reservationId: string) {
  ...
  const payments = await tx.select().from(providerPayments).where(eq(providerPayments.orderId, reservation.orderId));
  const paid = payments.filter((payment) => payment.status === "succeeded");
  const days = order.productSnapshot.entitlementDays;
  if (!inventory?.active || !site || paid.length !== 1 || payments.some((payment) => ["refunded", "disputed"].includes(payment.status))
    || typeof days !== "number" || !Number.isInteger(days) || days <= 0 || days > 36500) throw new AppError("CONFLICT", "Payment, ownership or placement needs reconciliation before approval.", 409);
  ...
  const startsAt = reservation.startsAt ?? sql`now()`, endsAt = reservation.endsAt ?? sql`now() + (${days} * interval '1 day')`;
  ...
  await tx.update(entitlements).set({ adSlotId: reservation.adSlotId, startsAt, endsAt, updatedAt: sql`now()` })
    .where(and(eq(entitlements.source, "dodo"), eq(entitlements.sourceId, `payment:${paid[0].providerPaymentId}`), eq(entitlements.kind, "AD_SLOT")));
