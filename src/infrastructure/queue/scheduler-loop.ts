...
export function startSchedulerLoop(tasks: SchedulerTasks, intervalMs: number) {
  ...
  const finished = (async () => {
    while (!closing) {
      try {
        ...
        const scheduling = await Promise.allSettled([tasks.scheduleDailyRetests(), tasks.scheduleMaintenance()]);
