...
import { ANALYTICS_CONSENT_COOKIE, isAnalyticsOrigin, isAnalyticsPage, readAnalyticsConsent, type AnalyticsConsent, type PublicAnalyticsConfig } from "./consent";
...
export default function ConsentAnalytics({ config }: { config: PublicAnalyticsConfig }) {
  ...
  const datafastClient = useRef<DataFastWeb | null>(null);
  const datafastLoading = useRef<Promise<DataFastWeb> | null>(null);
  const [consentError, setConsentError] = useState("");
  const pathname = usePathname(), params = useSearchParams();
  const search = params.toString();
  useEffect(() => {
    if (!config.enabled || !isAnalyticsOrigin(location.origin, config.siteOrigin)) return;
    const read = () => {
      ...
      if (value !== "granted" && trackersStarted.current) {
        void (async () => {
          try { const client = await datafastLoading.current; await (client ?? datafastClient.current)?.optOut(); } catch { /* Reload still removes listeners. */ }
          try { clearDataFastStorage(localStorage); clearDataFastStorage(sessionStorage); } catch { /* Storage can be blocked. */ }
          location.reload();
        })();
        return;
      }
  ...
  }, [config.enabled, config.siteOrigin]);
  const eligible = isAnalyticsPage(pathname, search);
  useEffect(() => {
    if (!config.enabled || !isAnalyticsOrigin(location.origin, config.siteOrigin) || consent !== "granted" || !eligible) return;
    ...
    if (config.datafastWebsiteId && config.datafastDomain && isAnalyticsReferrer(document.referrer)) {
      datafastLoading.current = loadDataFastClient(config.datafastWebsiteId, config.datafastDomain);
      void datafastLoading.current.then(async (client) => {
        datafastClient.current = client;
        // Consent or route can change while the npm chunk is loading.
        if (disposed || !isAnalyticsOrigin(location.origin, config.siteOrigin) || readAnalyticsConsent(document.cookie, browserSignals()) !== "granted"
          || !isAnalyticsPage(location.pathname, location.search) || !isAnalyticsReferrer(document.referrer)) return;
        await client.trackPageview(location.pathname);
        await client.flush();
      }).catch(() => { /* Optional measurement never interrupts website access. */ });
    }
    return () => { disposed = true; for (const script of scripts) script.remove(); history.pushState = push; history.replaceState = replace;
      if (config.gaId) win[`ga-disable-${config.gaId}`] = true; };
  }, [config.enabled, config.siteOrigin, config.gaId, config.datafastWebsiteId, config.datafastDomain, consent, eligible]);
  useEffect(() => {
    if (consent !== "granted" || !config.enabled || !isAnalyticsOrigin(location.origin, config.siteOrigin)) return;
  ...
  }, [consent, config.enabled, config.siteOrigin]);
  async function choose(value: "granted" | "denied") {
    ...
    if (choice === "denied") {
      // Finish any in-flight initialization before deleting SDK cookies/storage.
      try { const client = await datafastLoading.current; await (client ?? datafastClient.current)?.optOut(); } catch { /* Reload still unloads the SDK. */ }
      try { clearDataFastStorage(localStorage); clearDataFastStorage(sessionStorage); } catch { /* Storage may be unavailable. */ }
  ...
  if (!config.enabled || !loaded || !isAnalyticsOrigin(window.location.origin, config.siteOrigin) || pathname.startsWith("/unsubscribe")) return null;
  ...
  return <aside aria-label="Analytics preferences" className="fixed bottom-4 left-4 right-4 z-50 max-w-[34rem] rounded-2xl border border-border bg-bg-elevated p-5 shadow-pop sm:p-6">
    <p className="text-[15px] font-semibold leading-snug tracking-[-.01em] text-text-primary">Allow optional website analytics?</p>
